...
export const SettingBlockBannerCategories: React.FC<Props> = ({ setIsVisible }) => {
  ...
  useEffect(() => {
    setImages([selectedBlock?.category?.image || {}])
    setTitle(selectedBlock?.category?.name || '')
    setShowTitle(selectedBlock?.category?.visibleName)
    setShowImage(selectedBlock?.category?.image?.visible)
  }, [selectedBlock?.category?.image, selectedBlock?.category?.name, selectedBlock?.category?.visibleName])
  ...
  const onSave = useCallback(() => {
    onUpdateBlockItem({
      category: {
        ...
        name: title,
        visibleName: showTitle,
      },
    })
    setIsVisible?.(false)
    setCurrentIndexBlock(undefined)
  }, [images, onUpdateBlockItem, selectedBlock?.category, setCurrentIndexBlock, setIsVisible, showImage, showTitle, title])
      ...
      <Scrollbars className="h-full flex-1">
        <div className="h-full flex flex-col px-4">
          <div className="mb-4 flex items-center justify-between">
            <Typography.Paragraph className="font-medium text-gray800 mb-0">Show category name</Typography.Paragraph>
          ...
          <div className="mb-4 flex items-center justify-between">
            <Typography.Paragraph className="font-medium text-gray800 mb-0">Show category image</Typography.Paragraph>
